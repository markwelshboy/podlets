...
# sl:description Run Analyze+DWPose+SAM3D+Fusion+profile+Compose on a cached caption dataset and return a tar
# sl:output 5
# sl:setup-version 1
# sl:memcheck
...
sl_prepare() {
  ...
  mkdir -p "$QCAP_ROOT" "$QCAP_RUNS" "$(dirname "$QCAP_REPO")"
...
PY
  ...
  python3 - "$provenance" "$dataset_key" "$dataset_sha" "$run_name" "$subject_token" "$requested_ref" "$actual_sha" <<'PY'
...
PY
  ...
  echo "Running caption/projection regression tests"
  if ! (
    cd "$QCAP_REPO"
...
PY
  ...
  fusion_model_dir="$run_dir/fusion-v2.3/$model_slug"

  echo "Stage: SAM3D dataset profile"
  QWEN_WORKSPACE_ROOT="$QCAP_VLLM_WS" \
    bash "$QCAP_REPO/run_sam3d_dataset_profile_workspace.sh" \
      "$fusion_model_dir"

  echo "Stage: Compose A/B/C (8B BF16)"
  QWEN_WORKSPACE_ROOT="$QCAP_QWEN_WS" \
    bash "$QCAP_REPO/run_compose_fusion_compare_workspace.sh" \
      "$run_dir" \
